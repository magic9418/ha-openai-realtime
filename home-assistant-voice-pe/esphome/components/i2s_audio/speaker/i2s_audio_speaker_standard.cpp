...
// Voice PE runs the wake-word model, two WebSocket clients, Sendspin, and the
// reply pipeline at the same time. Five 10 ms stereo/32-bit DMA buffers consume
// 19,200 bytes of scarce DMA-capable internal RAM and fail once a voice session
// is active. Five 5 ms buffers retain 25 ms of hardware buffering while freeing
// 9,600 bytes for WebSocket RX and task allocations. The larger software audio
// rings remain in PSRAM, so this does not reduce burst absorption.
static constexpr uint32_t DMA_BUFFER_DURATION_MS = 5;
